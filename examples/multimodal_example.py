#!/usr/bin/env python3
"""
Example demonstrating multimodal functionality with vision-capable models

This example shows how to use the multimodal helper functions with both
the base Assistant class and ChatStreaming class to process images along
with text queries.
"""

import asyncio
from pathlib import Path

from langchain_llm_config import (
    create_assistant,
    create_chat_streaming,
    create_image_content,
    create_multimodal_query,
)


async def example_base_assistant_multimodal():
    """Example using base Assistant with multimodal input"""
    print("=== Base Assistant Multimodal Example ===")

    # Create assistant with auto_apply_parser=False for raw text output
    # Use a vision-capable model like gpt-4-vision-preview or gpt-4o
    assistant = create_assistant(
        provider="vlm",
        auto_apply_parser=False,  # Raw text output for multimodal
    )

    # Example 1: Using image URL
    print("\n1. Using image URL:")
    multimodal_query = create_multimodal_query(
        text="What do you see in this image? Describe it in detail.",
        image_url="https://www.mdxblog.io/images/posts/how-to-use-images/grass-tree-sky.jpg",
    )

    print(f"Query format: {multimodal_query}")
    # Uncomment to actually call the API:
    result = await assistant.ask_async(multimodal_query)
    print(f"Response: {result}")

    # Example 2: Using local image file (if it exists)
    print("\n2. Using local image file:")
    image_path = Path("sample_image.jpg")
    if image_path.exists():
        multimodal_query = create_multimodal_query(
            text="Analyze this image and tell me what you observe.",
            image_path=image_path,
        )
        print(f"Query format: {multimodal_query}")
        # Uncomment to actually call the API:
        # result = await assistant.ask_async(multimodal_query)
        # print(f"Response: {result}")
    else:
        print("Local image file not found, skipping this example")

    # Example 3: Using base64 encoded image
    print("\n3. Using base64 encoded image:")
    # This is a tiny 1x1 pixel PNG for demonstration
    tiny_png_base64 = ""

    multimodal_query = create_multimodal_query(
        text="What can you tell me about this image?", image_base64=tiny_png_base64
    )
    print(f"Query format: {multimodal_query}")
    # Uncomment to actually call the API:
    result = await assistant.ask_async(multimodal_query)
    print(f"Response: {result}")


async def example_chat_streaming_multimodal():
    """Example using ChatStreaming with multimodal input"""
    print("\n\n=== ChatStreaming Multimodal Example ===")

    # Create streaming chat with vision-capable model
    chat = create_chat_streaming()

    # Example: Streaming response for image analysis
    print("\n1. Streaming multimodal response:")
    multimodal_query = create_multimodal_query(
        text="Describe this image step by step, focusing on colors, objects, and composition.",
        image_url="https://upload.wikimedia.org/wikipedia/commons/thumb/d/dd/Gfp-wisconsin-madison-the-nature-boardwalk.jpg/2560px-Gfp-wisconsin-madison-the-nature-boardwalk.jpg",
    )

    print(f"Query format: {multimodal_query}")

    # Uncomment to actually stream the response:
    # print("Streaming response:")
    # async for chunk in chat.chat_stream(multimodal_query):
    #     if chunk["type"] == "stream":
    #         print(chunk["content"], end="", flush=True)
    #     elif chunk["type"] == "final":
    #         print(f"\n\nFinal response received in {chunk['processing_time']:.2f}s")
    #     elif chunk["type"] == "error":
    #         print(f"Error: {chunk['error']}")

    # Example: Non-streaming multimodal response
    print("\n2. Non-streaming multimodal response:")
    # Uncomment to actually call the API:
    result = await chat.chat(multimodal_query)
    print(f"Response: {result['content']}")
    print(f"Processing time: {result['processing_time']:.2f}s")


def example_helper_functions():
    """Example demonstrating the helper functions"""
    print("\n\n=== Helper Functions Examples ===")

    # Example 1: create_image_content function
    print("\n1. create_image_content examples:")

    # URL image
    url_content = create_image_content(image_url="https://example.com/image.jpg")
    print(f"URL image content: {url_content}")

    # Base64 image
    base64_content = create_image_content(
        image_base64="iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="
    )
    print(f"Base64 image content: {base64_content}")

    # Example 2: create_multimodal_query function
    print("\n2. create_multimodal_query examples:")

    # Text only
    text_only = create_multimodal_query("Just a text question")
    print(f"Text-only query: {text_only}")

    # Text + image
    text_image = create_multimodal_query(
        text="What's in this image?", image_url="https://example.com/image.jpg"
    )
    print(f"Text + image query: {text_image}")


async def main():
    """Run all examples"""
    print("Multimodal Functionality Examples")
    print("=" * 50)

    # Run helper function examples (no API calls)
    example_helper_functions()

    # Run assistant examples (API calls commented out)
    await example_base_assistant_multimodal()
    await example_chat_streaming_multimodal()

    print("\n" + "=" * 50)
    print("Examples completed!")
    print("\nTo actually call the APIs:")
    print("1. Ensure you have a valid OpenAI API key set")
    print("2. Uncomment the API call lines in the examples")
    print("3. Make sure you're using a vision-capable model (e.g., gpt-4o)")


if __name__ == "__main__":
    asyncio.run(main())
